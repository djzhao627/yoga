...
iTsai.form = {
    ...
    _serializeGroups : function(groups) {
        ...
        for ( var i = groups.length - 1; i >= 0; i--) {
            ...
            var inputs = group
                    .find("input[type!=button][type!=reset][type!=submit][id!='__VIEWSTATE'][id!='__EVENTVALIDATION'],select,textarea");
    ...
    serialize : function(frm) {
        ...
        var inputs = frm
                .find("input[type!=button][type!=reset][type!=submit][type!=image][id!='__VIEWSTATE'][id!='__EVENTVALIDATION'],select,textarea");
    ...
    _filterInputs : function(container) {
        var inputs = $(container
                .find("input[type!=button][type!=reset][type!=submit][type!=image][type!=file][id!='__VIEWSTATE'][id!='__EVENTVALIDATION'],select,textarea"));
        return inputs;
    },
    /**
     * 查找符合条件的输入标签
    ...
    _findInputs : function(inputs, key) {
        var input = $(inputs.filter("input[name=" + key + "][id!='__VIEWSTATE'][id!='__EVENTVALIDATION'],input[id=" + key
...
//将iTsai.form.serialize绑定到jQuery对象上，可以直接在jQuery对象上调用。如：$('#fr').serializeObject();
//此部分代码也可以删除。
(function ($) {

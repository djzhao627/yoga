...
(function (window, $, undefined) {
    ...
    var version = vt.version,
        core_array = [],
        core_trim = jQuery.trim(version),//.trim,
    ...
    coreString.replaceAll = function (str, substr, replacement, ignoreCase) {
        ...
        var regexp = coreUtil.isRegExp(substr) ? substr : new RegExp(String(substr), ignoreCase ? "g" : "g");
    ...
    coreString.isMobile = function (str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        //原：return /^(\d{1,4}\-)?(13|14|15|17|18)\d{9}$/i.test(str);
        return /^(\d{1,4}\-)?(1)\d{10}$/i.test(str);
    };
    ...
    //  判断当前 String 对象是否可以作为 简单的安全密码字符(由字母和数字组成，6~20 位).
    coreString.isSimpleSafePwd = function (str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        return (/^(([A-Z]|[a-z]|\d){6,20})$/.test(str));
    };
    coreString.prototype.isSimpleSafePwd = function () { return coreString.isSimpleSafePwd(this); };
    
    //  判断当前 String 对象是否可以作为 控制器ID字符(由字母+数字组成，1~16 位).
    coreString.isControllerId = function (str) {
    	str = coreString.isNullOrEmpty(str) ? "" : String(str);
    	return (/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{1,16}$/.test(str));
    };
    coreString.prototype.isControllerId = function () { return coreString.isControllerId(this); };
        ...
        if ((array1 && typeof array1 === "object" && array1.constructor === Array) && (array2 && typeof array2 === "object" && array2.constructor === Array)) {
            if (array1.length == array2.length) {
                for (var i = 0; i < array1.length; i++) {
                    var ggg = jQuery.compObj(array1[i], array2[i]);
                    if (!ggg) {
                        return false;
                    }
...
jQuery.extend({
    compObj: function (obj1, obj2)//比较两个对象是否相等，不包含原形上的属性计较
    {
        if ((obj1 && typeof obj1 === "object") && ((obj2 && typeof obj2 === "object"))) {
            var count1 = jQuery.propertyLength(obj1);
            var count2 = jQuery.propertyLength(obj2);
            if (count1 == count2) {
                for (var ob in obj1) {
                    if (obj1.hasOwnProperty(ob) && obj2.hasOwnProperty(ob)) {
                        ...
                        {
                            if (!jQuery.compArray(obj1[ob], obj2[ob])) {
                                return false;
                            };
                        }
                        ...
                        {
                            if (!jQuery.compObj(obj1[ob], obj2[ob])) {
                                return false;
                            };
                        }
                        else {
